...
FaceAR::FaceAR()
{
    FaceARParameters parameters;
	this->Read(parameters.model_location);
}

FaceAR::FaceAR(string fname)
{
	this->Read(fname);
}

FaceAR::FaceAR(const FaceAR& other): pdm(other.pdm), params_local(other.params_local.clone()), params_global(other.params_global), detected_landmarks(other.detected_landmarks.clone()),
...
FaceAR & FaceAR::operator= (const FaceAR& other)
{
    if (this != &other)
	{
		pdm = PDM(other.pdm);
...
FaceAR::FaceAR(const FaceAR&& other)
{
	this->detection_success = other.detection_success;
	this->tracking_initialised = other.tracking_initialised;
	this->detection_certainty = other.detection_certainty;
...
void FaceAR::Read_FaceAR(string facear_location)
{
    ifstream locations(facear_location.c_str(), ios_base::in);

	if(!locations.is_open())
	{
        cout << "Couldn't open the FaceAR model file aborting" << endl;
		cout.flush();
		return;
	}
    ...
    boost::filesystem::path root = boost::filesystem::path(facear_location).parent_path();
	while (!locations.eof())
	{ 
		...
		{       
            ...
            FaceARTracker::SkipComments(triangulationFile);
			...
			{
                FaceARTracker::SkipComments(triangulationFile);
                FaceARTracker::ReadMat(triangulationFile, triangulations[i]);
			}
            //cout << "Done" << endl;
		}
...
void FaceAR::Read(string main_location)
{

    //cout << "Reading the FaceAR landmark detector/tracker from: " << main_location << endl;
	...
	{ 
		...
		// append to root
		location = (root / location).string();
        if (module.compare("FaceAR") == 0)
		{ 
            //cout << "Reading the FaceAR module from: " << location << endl;
            Read_FaceAR(location);
		}
        else if(module.compare("FaceAR_part") == 0)
		{
			string part_name;
    ...
    // TODO: tracking_initialised
    tracking_initialised = true; //false;
...
void FaceAR::Reset(double x, double y)
{
	this->Reset();
...
bool FaceAR::DetectLandmarks(const Mat_<uchar> &image, const Mat_<float> &depth, FaceARParameters& params)
{
...
//=============================================================================
bool FaceAR::Fit(const Mat_<uchar>& im, const Mat_<float>& depthImg, const std::vector<int>& window_sizes, const FaceARParameters& clm_parameters)
{
    ...
    FaceARParameters tmp_parameters = clm_parameters;
	for(int scale = 0; scale < num_scales; scale++)
	{
		...
		{
            cout << "Detection too small for FaceAR" << endl;
			return false;
		}
	}
...
void FaceAR::NonVectorisedMeanShift_precalc_kde(Mat_<float>& out_mean_shifts, const vector<Mat_<float> >& patch_expert_responses, const Mat_<float> &dxs, const Mat_<float> &dys, int resp_size, float a, int scale, int view_id, map<int, Mat_<float> >& kde_resp_precalc)
{
	...
	for(int i = 0; i < n; i++)
	{
        ...
        int closest_col = (int)(dy /step_size + 0.5);
        int closest_row = (int)(dx /step_size + 0.5);
		
        int idx = closest_row * ((int)(resp_size/step_size + 0.5)) + closest_col;
...
void FaceAR::GetWeightMatrix(Mat_<float>& WeightMatrix, int scale, int view_id, const FaceARParameters& parameters)
{
	int n = pdm.NumberOfPoints();  
...
//=============================================================================
double FaceAR::NU_RLMS(Vec6d& final_global, Mat_<double>& final_local, const vector<Mat_<float> >& patch_expert_responses, const Vec6d& initial_global, const Mat_<double>& initial_local,
                  ...
                  const FaceARParameters& parameters)
{		
...
bool FaceAR::RemoveBackground(Mat_<float>& out_depth_image, const Mat_<float>& depth_image)
{
...
Mat_<double> FaceAR::GetShape(double fx, double fy, double cx, double cy) const
{
    ...
    Matx33d R = FaceARTracker::Euler2RotationMatrix(Vec3d(params_global[1], params_global[2], params_global[3]));
    ...
    // this is described in the paper in section 3.4 (equation 10) (of the FaceAR-Z paper)
	for(int i = 0; i < n; i++)
	{
...
Rect_<double> FaceAR::GetBoundingBox() const
{
...
void FaceAR::NonVectorisedMeanShift(Mat_<double>& out_mean_shifts, const vector<Mat_<float> >& patch_expert_responses, const Mat_<double> &dxs, const Mat_<double> &dys, int resp_size, double a, int scale, int view_id)
{
