 ...
 */
	 ...
	 */
    render::Mesh MorphableModel::get_mean() const
	{
	 ...
	 */
    render::Mesh MorphableModel::draw_sample(float shape_sigma = 1.0f, float color_sigma = 1.0f)
	{
	 ...
	 */
    render::Mesh MorphableModel::draw_sample(std::vector<float> shape_coefficients, std::vector<float> color_coefficients)
	{
		assert(shape_model.get_data_dimension() == color_model.get_data_dimension() || !has_color_model()); // The number of vertices (= model.getDataDimension() / 3) has to be equal for both models, or, alternatively, it has to be a shape-only model.
	 ...
	 */
    bool MorphableModel::has_color_model() const
	{
	 ...
	 */
    std::vector<cv::Vec2f> MorphableModel::get_texture_coordinates() const
	{
