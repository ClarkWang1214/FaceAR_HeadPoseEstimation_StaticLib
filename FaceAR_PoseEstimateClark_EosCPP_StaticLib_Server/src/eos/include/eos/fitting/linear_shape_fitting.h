...
inline std::vector<float> fit_shape_to_landmarks_linear(morphablemodel::MorphableModel morphable_model,
                                                        cv::Mat affine_camera_matrix, std::vector<cv::Vec2f> landmarks,
                                                        std::vector<int> vertex_ids, float lambda=3.0f,
                                                        ...
                                                        boost::optional<float> model_standard_deviation=boost::optional<float>())
{
    ...
    // 63  m x n (rows x cols) = numShapeDims x numShapePcaCoeffs, (=eigenvector matrix V). Each column is an eigenvector.
...
//    std::cout << "V_hat_h : "<< std::endl << V_hat_h << std::endl;
//    cv::imshow("V_hat_h", V_hat_h);
//    cv::waitKey(33);
    ...
    float sigma_squared_2D = std::pow(detector_standard_deviation.get_value_or(std::sqrt(3.0f)), 2) + std::pow(model_standard_deviation.get_value_or(0.0f), 2); ////TODO
